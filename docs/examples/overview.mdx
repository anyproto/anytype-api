---
title: Overview
sidebar_position: 1
---

# Overview

Discover practical examples and projects that show different ways to connect with the Anytype API.

---

## Featured

Explore official projects from the Anytype team that highlight best practices and showcase what’s possible:

- **[Model Context Protocol (MCP) Server](/docs/examples/featured/mcp)** - Integrate Anytype with AI assistants using MCP
- **[Command-Line Interface (CLI)](/docs/examples/featured/cli)** - Run Anytype as a headless server for automation and scripting
- **[Raycast Extension](/docs/examples/featured/raycast)** - A Raycast extension for managing your Anytype workspace.
- **[Daily Journal](/docs/examples/featured/journal)** - Create and manage a daily journal in Anytype

---

## Community

Projects by the community showcasing creative implementations and use cases.

**→ [Submit your project](https://github.com/anyproto/anytype-api/blob/main/data/README.md)** - Share what you've built with the Anytype API!

### ⭐ Most Popular

Projects sorted by GitHub stars:

- **[Python Client for Anytype](/docs/examples/community/charlesneimog-anytype-client)** - ⭐ 106 stars
- **[Save to Anytype (Browser Extension)](/docs/examples/community/lazjedi-save-to-anytype)** - ⭐ 40 stars
- **[Anytype Toolbox](/docs/examples/community/stevelr-anyr)** - ⭐ 32 stars
- **[Anytype Rust SDK](/docs/examples/community/stevelr-anytype)** - ⭐ 32 stars
- **[Anytype Go SDK](/docs/examples/community/epheo-anytype-go)** - ⭐ 28 stars

### 📁 By Category

#### Integrations

- **[Anytype Toolbox](/docs/examples/community/stevelr-anyr)** (⭐ 32) - CLI and MCP Server for Anytype: manage spaces and objects, edit documents in Markdown, back up and restore, chat
- **[Anytype MCP (Semantic Search)](/docs/examples/community/wethegreenpeople-anytype-mcp)** (⭐ 18) - A Model Context Protocol (MCP) server for Anytype that enables semantic search and RAG (Retrieval-Augmented Generation) capabilities over your Anytype documents
- **[Anytype LangChain Document Loader](/docs/examples/community/littlehongman-anytype-loader)** (⭐ 6) - Load documents from Anytype spaces into LangChain format. Supports sync/async operations with pagination and search.
- **[n8n Anytype Node](/docs/examples/community/splch-n8n-nodes-anytype)** (⭐ 27) - An n8n community node for the Anytype API.

#### SDKs

- **[Anytype Rust SDK](/docs/examples/community/stevelr-anytype)** (⭐ 32) - An ergonomic Anytype API client in Rust. Unifies HTTP and gRPC backends.
- **[Python Client for Anytype](/docs/examples/community/charlesneimog-anytype-client)** (⭐ 106) - A Python client library for interacting with the Anytype API
- **[Anytype Go SDK](/docs/examples/community/epheo-anytype-go)** (⭐ 28) - A Go SDK for interacting with the Anytype API to manage spaces, objects, and perform searches. This library provides a feature-rich, fluent interface to integrate Anytype functionality into your Go applications
- **[Anytype Python Client](/docs/examples/community/beaucronin-anytype-python-client)** (⭐ 17) - A comprehensive Python client for the Anytype API with both synchronous and asynchronous support
- **[Anytype Rust Library](/docs/examples/community/lanesawyer-anytype_rs)** (⭐ 6) - Rust library for the Anytype API
- **[Anytype.NET](/docs/examples/community/glabular-anytype-net)** (⭐ 6) - An unofficial .NET client library for accessing the Anytype API

#### Tools

- **[Global Search CLI](/docs/examples/community/mowbraym-anytype-global-search)** (⭐ 0) - A command line utility that accesses the Anytype API to provide global (cross-Space) search
- **[AnyTask](/docs/examples/community/tumypmyp-any-task)** (⭐ 7) - A Cross-Platform Client for Anytype written with Rust and Dioxus

#### Extensions

- **[Save to Anytype (Browser Extension)](/docs/examples/community/lazjedi-save-to-anytype)** (⭐ 40) - Save important web pages with your selected text in Markdown format to Anytype. Lightweight and easy-to-use alternative web clipper.

#### Templates...

*This could be your next project! [Submit it here](https://github.com/anyproto/anytype-api/blob/main/data/README.md)*
