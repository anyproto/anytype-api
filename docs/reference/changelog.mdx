...
- Deprecated authentication endpoints `/v1/auth/display_code` and `/v1/auth/token` in favor of the challenge-based system (`/v1/auth/challenges` and `/v1/auth/api_keys`) introduced in 2025-05-20
...
- Authentication endpoints (`/v1/auth/display_code`, `/v1/auth/token`) will be deprecated in favor of the new challenge-based system in a future release
...
- Returned linked `properties` (featured and from sidebar) and `plural_name` for types
...
- Added `Anytype-Version` response header
- Added `name` as a sort property option for search endpoints
- Added `description` field for spaces
- Included custom `properties` (formerly relations) in object response details

### Changed

- Upgraded the Swagger specification to OpenAPI 3.1
